...
import { set, nextThursday, formatDistanceStrict, isThursday } from "date-fns"
...
export async function execute(interaction: CommandInteraction) {
  ...
  if (isThursday(now)) {
    upcoming = set(now, { hours: 19, minutes: 0, seconds: 0, milliseconds: 0 })
  } else {
    upcoming = set(nextThursday(now), { hours: 19, minutes: 0, seconds: 0, milliseconds: 0 })
  }
